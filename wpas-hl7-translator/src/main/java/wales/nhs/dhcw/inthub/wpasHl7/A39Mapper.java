...
public class A39Mapper {
    ...
    public ADT_A39 ADT_A39Mapper(MAINDATA.TRANSACTION transaction) throws DataTypeException {
        if(MshMapper(transaction)!=null) {
            if(EVNMapper(transaction)!=null) {
                if(PIDMapper(transaction)!=null){
                    if(PD1Mapper(transaction)!=null){
                        if(MRGMapper(transaction)!=null){
    ...
    public ADT_A39 MshMapper(MAINDATA.TRANSACTION transaction) throws DataTypeException {
        ...
        try {
            ...
            adtMessage.getMSH().getDateTimeOfMessage().getTs1_Time().setValue(sdf.format(now));//yyyyMMddHHmmss
    ...
    public ADT_A39 EVNMapper(MAINDATA.TRANSACTION transaction) throws DataTypeException {
        ...
        SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
        try {
            adtMessage.getEVN().getEvn2_RecordedDateTime().getTs1_Time().setValue(sdf.format(now));
            adtMessage.getEVN().getEvn5_OperatorID(0).getXcn1_IDNumber().setValue(transaction.getUSERID());
            adtMessage.getEVN().getEvn6_EventOccurred().getTs1_Time().setValue(sdf.format(now));
    ...
    public ADT_A39 PIDMapper(MAINDATA.TRANSACTION transaction) throws DataTypeException {
        ...
        DateTimeFormatter inputDateTimeFormatter = DateTimeFormatter.ISO_LOCAL_DATE_TIME;
        ...
        try {
            ...
            adtMessage.getPATIENT().getPID().getPid16_MaritalStatus().getCe1_Identifier().setValue(transaction.getMARITALSTATUS().toString());
            ...
            LocalDateTime dateTime = LocalDateTime.parse(transaction.getUPDATEDATE(), inputDateTimeFormatter);
            adtMessage.getPATIENT().getPID().getPid33_LastUpdateDateTime().getTs1_Time().setValue(dateTime.format(outputFormatter));//Need to change
    ...
    public ADT_A39 MRGMapper(MAINDATA.TRANSACTION transaction) throws DataTypeException {
        try {
            adtMessage.getPATIENT().getMRG().getMrg1_PriorPatientIdentifierList(0).getCx1_IDNumber().setValue(transaction.getOLDUNITNUMBER().toString());
